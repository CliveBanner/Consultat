...
public class CustomerService {
	 ...
	 */
	public synchronized List<Customer> findAll(String... stringFilters) {
		ArrayList<Customer> arrayList = new ArrayList<>();
		for (Customer contact : contacts.values()) { 
			int passCounter = 0;
			for(String filter: stringFilters) {
				boolean passesFilter = false;
				passesFilter = (filter == null || filter.isEmpty())
						|| contact.toString().toLowerCase().contains(filter.toLowerCase());
				if (passesFilter) {
					passCounter++;
				}
			}
			if(passCounter==stringFilters.length) {
				try {
	...
	public void ensureTestData() {
		if (findAll().isEmpty()) {
			final String[] names = new String[] { "Friseur Janik Bochumerstr. 44629 Herne", "Nagelstudio GutFoot Sodingerstr. 44625 Herne", "Sonnenstudio Branka Gotherstr. 44629 Herne" };
			Random r = new Random(0);
			for (String name : names) {
				Customer c = new Customer();
				String[] data = name.split(" ");
				c.setBusiness(data[0]);
				c.setName(data[1]);
				c.setStreet(data[2]);
				c.setPlz(data[3]);
				c.setTown(data[4]);
